...
function populateTable(orders) {
    ...
    orders.forEach((order, index) => {
        ...
        let previousState = document.createElement("span");
        previousState.textContent = orderStateToText(order.PreviousState) + "->";

        cellState.appendChild(previousState);
...
function deleteOrder(orderId) {
    ...
    axios.post("/MainOrder/DeleteOrder", { orderIdDto })
        .then(response => {
            let errorCode = response.data.errorCode;
